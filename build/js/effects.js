...
function fixedHeader () {
    $(window).scroll(function(){
        if ( $(this).scrollTop() > 30 && $("header").hasClass("default") ){
            $("header").removeClass("default", "headerActiveToggle").addClass("fixed");
        } else if($(this).scrollTop() <= 30 && $("header").hasClass("fixed")) {
            $("header").removeClass("fixed", "headerActiveToggle").addClass("default");
        }
    });
}
...
function toggleMenu() {
    $('.toggle').click(function () {
        if (!$('ul > li').hasClass('liToggle')){
            ...
            $("header").addClass('headerActiveToggle');

            $('a').removeClass('active');
            $('ul > li').addClass('liToggle');

            headerBackground(); //call header remove background color function
        } else {
            $('ul > li').removeClass('liToggle');
            $("header").removeClass('headerActiveToggle');
...
function headerBackground() {
    $('a[href^="#home"]').bind("click", function(e){
        $("header").removeClass('headerActiveToggle');
    });
}
...
function removeEffectsTextH () {
    ...
    if (window.matchMedia('screen and (min-width: 768px) and (max-width: 991px)').matches) {
        $('.portfolio-item').removeAttr('data-aos');
    }
}
